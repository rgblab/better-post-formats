...
add_filter( 'bpf_set_style', function () {
	$style = '';

	$content_width_list = get_option( BPF_OPTIONS )['content_width_list'] ? get_option( BPF_OPTIONS )['content_width_list'] : '100%';
	$style              .= '.bpf-content__grid{width:' . esc_attr( $content_width_list ) . '}';

	$content_width_single = get_option( BPF_OPTIONS )['content_width_single'] ? get_option( BPF_OPTIONS )['content_width_single'] : '1200px';
